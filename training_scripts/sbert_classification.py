...
# SBERT model with classification head
class SBERTClassifier(nn.Module):
    def __init__(self, model_name="sentence-transformers/all-MiniLM-L6-v2", num_labels=2):
        ...
        self.classifier = nn.Linear(384, num_labels)  # 384 is the embedding size for all-MiniLM-L6-v2
...
# Save the model
output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../models/fine_tuned_sbert')
...
def measure_deployability_metrics(model, test_dataset, batch_size=32):
    ...
    batch_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False)
...
metrics_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../results/sbert_deployability_metrics.json')
